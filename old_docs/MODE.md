...
이 섹션은 과거 설계 기준 설명입니다. 현재 구현에서는:

- 네트워크 모드(`loopback` / `namespace` / `external_port`)는 시험 프로파일이 아니라 서버 런타임 설정입니다.
- 시험 프로파일에는 네트워크 모드 대신 `tcp_options.tcp_quickack`만 저장됩니다.
