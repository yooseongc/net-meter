...
### 6. `server_map()` / `client_map()` 매 호출마다 clone ✅ 완료
**위치:** `core/src/config.rs`

**문제:** 두 메서드가 호출될 때마다 전체 HashMap을 새로 생성.
Generator `start()` 와 Orchestrator에서 여러 번 호출될 경우 불필요한 복사.

**해결:** 반환 타입을 `HashMap<&str, &ServerDef>` / `HashMap<&str, &ClientDef>`로 변경.
호출 측(`generator/src/lib.rs`, `control/src/orchestrator.rs`, `local_server_addrs()`)에서
`.get(id.as_str())` 형태로 업데이트.
